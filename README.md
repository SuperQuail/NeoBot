...
**官方娱乐插件 `starship`** 把控制台搬进一艘可自由探索的 3D 科幻战舰（three.js 第一人称、MC 式操作）：

- 与面板**共用端口**：面板侧栏「星舰」入口进入（默认 `http://<面板地址>/game/`）；
- 8 个菜单都是舰上的全息终端（主控台 / 模块机架 / 舰载系统配置 / 反应堆 / 能量分配 / 神经矩阵 / 通讯台 / 航行日志），功能与面板一致，危险操作在游戏内二次确认；
- 待机（低功耗）时用量 / 分析 / 通讯终端会熄屏下线，并给出原因与恢复指引；
- 舰外是实时渲染的太空：星云、行星、小行星流、过往飞船，以及可手动/自动触发的**跃迁**；
- 两个小游戏：舱外炮塔（world 模式）与损管抢修（全息屏解谜），成绩落库并排行；
- **不进入游戏就没有额外开销**：服务端没有任何后台任务，部署也不需要 Node（前端产物随包分发）。

详见 [docs/04-功能文档/星舰游戏.md](docs/04-功能文档/星舰游戏.md)；插件依赖体系（版本约束、自动禁用、前置能力调用）见 [docs/07-插件开发.md](docs/07-插件开发.md)。
