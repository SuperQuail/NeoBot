...
- 页面包含：仪表盘（运行概览/系统资源/消息趋势/最近日志）、插件管理（官方与第三方分组、启停、热重载、安装/更新/卸载、在线配置）、配置管理（`config.toml` 表单/TOML 编辑、`.env` 编辑、一键添加 API 供应商、模型库、模型分配）、系统状态（进程资源/宿主服务/后台任务/模型用量）、机器人详情、日志。
