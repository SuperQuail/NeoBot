...
存储或更新**立绘**（角色图/人设图/用户形象等）时，如果该立绘与某个 QQ 用户强相关——
例如该用户的角色/OC/人设、为用户本人或其亲友绘制的形象、聊天中明确属于某人的立绘——
**必须在描述中明确写入对应相关者的 QQ 号**，格式：

```
相关者QQ:123456789
```

- 可放在描述开头或结尾；同一人的多张立绘保持一致的写法，便于搜索检索
- 只有完全不与任何 QQ 用户相关的立绘（风景/静物/通用角色等）才不需要标注
- 立绘入库时就应写入；遗漏时通过 `gallery_update` 补充
