"""prompt/builder 模块（模板构建、占位符容错、关键词快照、自适应提示词）测试。"""
...
def _make_group_config(template: str) -> BotConfig:
    config = BotConfig()
    config.bot.nick_name = "小测试"
    config.bot.account = 10001
    config.bot.alias_name = ["测试", "  ", "T"]
    config.chat.group_description = {"123456": "测试群描述"}
    config.chat.group_prompt_template = template
    return config


def _make_friend_config(template: str) -> BotConfig:
    config = BotConfig()
    config.bot.nick_name = "小测试"
    config.bot.account = 10001
    config.bot.alias_name = ["测试", "T"]
    config.chat.friend_prompt_template = template
    return config
...
async def test_build_group_prompt_renders_all_template_placeholders():
    ...
    config = _make_group_config(
        ...
        "|{group_description}|{current_time}|{message_list}|{member_list}"
    )
    builder = PromptBuilder(config, _FakeProfileService())
...
async def test_build_group_prompt_appends_extra_fragments_when_placeholders_missing():
    ...
    # Arrange
    config = _make_group_config("你好{bot_name} {message_list}")
    config.chat.key_word = [
        {
            "enabled": True,
            "keywords": ["妈妈"],
...
async def test_build_group_prompt_tolerates_unknown_placeholder():
    ...
    # Arrange
    config = _make_group_config("你好{bot_name} {custom_placeholder}")
    builder = PromptBuilder(config, _FakeProfileService())
...
async def test_build_friend_prompt_renders_and_appends_memory_and_private_tips():
    ...
    # Arrange
    config = _make_friend_config(
        "你好{bot_name}（{friend_name}）{remark} {profile} {friend_info} {message_list}"
    )
    builder = PromptBuilder(config, _FakeProfileService())
...
async def test_keyword_rules_snapshot_survives_config_replacement():
    ...
    # Arrange
    config = _make_group_config("你好{bot_name} {message_list} {key_word_reaction_list}")
    config.chat.key_word = [
        {
            "enabled": True,
            "keywords": ["妈妈"],
...
async def test_adaptive_prompt_ignored_when_path_absent_or_missing(tmp_path):
    ...
    # Arrange
    config = _make_group_config("你好{bot_name} {message_list}")
    no_path_builder = PromptBuilder(config, _FakeProfileService())
    missing_path_builder = PromptBuilder(
        config, _FakeProfileService(), adaptive_prompt_path=tmp_path / "nope.txt"
    )
