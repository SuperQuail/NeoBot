...
@pytest.mark.asyncio
async def test_pending_registry_cleaned_after_parsing_completes():
    service = ImageParseService()
    ...
    for group_id in ("11", "22", "33"):
        message = _group_message(int(group_id) * 100, ("image", {"file": "x.jpg"}))
        await service.parse_message_images(message, group_id)

    assert set(service._pending) == {"11", "22", "33"}
    ...
    for group_id in ("11", "22", "33"):
        await service.wait_for_queue(group_id)
    assert service._pending == {}


@pytest.mark.asyncio
async def test_wait_for_queue_cleans_registered_tasks():
    service = ImageParseService()
    service._parse_single_image = AsyncMock(return_value="[图片：自动解析结果]")
    message = _group_message(5001, ("image", {"file": "x.jpg"}))
    await service.parse_message_images(message, "50")
    await service.wait_for_queue("50")
    assert service._pending == {}
